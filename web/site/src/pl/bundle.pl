...
%
%     Cogbot's install directory, asserted by the handler
%     for the config info
:- dynamic cogbot_install_directory/1.

%
%     The list of parameters in key=value format (not urlencoded)
%     returned by the config page
:- dynamic config_data/1.

%
%%	cogbot_bin_dir(Dir)
%
%	directory path to Cogbot's bin directory
%	That is, this is a Prolog style path like
%	c:/development/opensim4opencog/bin/
%	ending in / that
%	will name the directory that Cogbot.exe can
%	be found in
cogbot_bin_dir(Bin) :-
	cogbot_install_directory(Install),
	atomic_concat(Install, 'bin/' , Bin).
...
add_order_constraints(AllPairs, AName-BName) :-
	member(AVar-AName, AllPairs),  % sneaky nested loops!
