...
component_will_install(X) :- select_status(X, selected).
component_will_install(X) :- select_status(X, needed).
...
select_status_html(X) -->
	...
	html([
	       ...
	       p('Locked because it is required by ~w'-[YName, YName])])
	    ]).
